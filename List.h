...
namespace cop4530 {
	...
	template <typename T>
		class List {
			...
			public:
				...
				void reverse(); // reverse the order of the elements, check
				...
				void remove(const T &val); // remove all elements with value = val, check
	...
	template <typename T>
		bool operator==(const List<T> & lhs, const List<T> &rhs);//check
	...
	template <typename T>
		std::ostream & operator<<(std::ostream &os, const List<T> &l);//check
